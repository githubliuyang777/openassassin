<template>
  <div>
      ...
      <n-grid-item>
        <n-card size="small">
          ...
          <div style="margin-top:8px">
            <n-text strong style="font-size:18px">{{ latest?.load_1m || '-' }}</n-text>
            <n-text depth="3" style="font-size:12px;margin-left:4px">1m</n-text>
            <n-text style="font-size:14px;margin-left:8px">{{ latest?.load_5m || '-' }}</n-text>
            <n-text depth="3" style="font-size:12px;margin-left:2px">5m</n-text>
            <n-text style="font-size:14px;margin-left:8px">{{ latest?.load_15m || '-' }}</n-text>
            <n-text depth="3" style="font-size:12px;margin-left:2px">15m</n-text>
          </div>
        </n-card>
      </n-grid-item>
    </n-grid>
    ...
    <n-card size="small" title="历史趋势" style="margin-bottom:16px">
      ...
      <div v-else ref="chartEl" style="width:100%;height:200px"></div>
    </n-card>
...
import { fetchHost, fetchHostMetrics, fetchLatestMetrics, regenerateAgentToken, updateHost } from '@/api/hosts'
...
function drawChart() {
  ...
  const yScale = (v: number) => padding.top + ph - ((v - minVal) / (maxVal - minVal)) * ph
  ...
  // Legend
  const legendY = h - padding.bottom + 26
  ...
  for (const s of series) {
    ...
    ctx.fillText(s.label, legendX + 13, legendY + 3)
    ...
    onPositiveClick: async () => {
      try {
        ...
        dialog.success({
          ...
          content: token,
          positiveText: '复制',
          onPositiveClick: () => navigator.clipboard.writeText(token).catch(() => {}),
        })
      } catch (e: any) {
...
async function handleCopyInstallCmd() {
  try {
    const resp = await import('@/api/hosts').then(m => m.fetchAgentToken(hostId.value))
    const token = resp.data.agent_token
    const origin = window.location.origin
    const cmd = `curl -fsSL ${origin}/install-agent.sh | bash -s -- --server ${origin} --token ${token}`
    navigator.clipboard.writeText(cmd).then(
      () => message.success('安装命令已复制'),
      () => message.warning('复制失败，请手动复制'),
    )
  } catch (e: any) {
    message.error(e.message || '获取 Token 失败')
  }
}
